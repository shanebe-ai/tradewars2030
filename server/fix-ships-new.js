...
async function fixShips() {
  ...
  console.log('✓ Cleared existing ship types');
  ...
  // Verify
  const result = await query('SELECT name FROM ship_types ORDER BY name');
  console.log('Ship types in database:');
  result.rows.forEach(ship => console.log(`  ${ship.name}`));
